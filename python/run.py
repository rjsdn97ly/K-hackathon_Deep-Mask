...
while True:
	...
	frame = imutils.resize(frame, width=1066)
...
#			maskStatus = True
		elif withoutMask > 0.3:
			label = "No Mask"
			color = (0, 0, 255)
			cv2.putText(frame, "ACCESS DENIED, No FACE-MASK", (10, 50), font4, 0.8, color, 2, cv2.LINE_AA)
#			maskStatus = False
		else:
			label = "Please, make sure to wear the MASK"
			color = (255, 255, 255)
			cv2.putText(frame, "Please, make sure to wear the MASK", (10, 50), font4, 0.8, color, 2, cv2.LINE_AA)
...
#		if maskStatus == True:
#			dir.update({'MASK':'MASK'})
#		elif maskStatus == False:
#			dir.update({'MASK':'UNMASK'})
